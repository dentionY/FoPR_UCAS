...
while conditional_back>theta
    ...
    for Alli = 1
         ...
         randomindex = ceil(rand(1,1)*21);
              for Hi = 1:1:H
                 Hidden_layer_add(Alli,Hi) = All_add(randomindex,:)*w_ih(:,Hi);
         ...
         %前向的第二步，计算输出层得到的net_j和激励值，输出用Sigmoid函数
          ...
          %前向的第三步，更新Jw
              minus_vec_tmp = (Out_layer_sti(Alli,:)-Target_add(randomindex,:)).*(Out_layer_sti(Alli,:)-Target_add(randomindex,:));
              Jw_tmp = sum(minus_vec_tmp)/2;
              if iteration == 1
                  Jw = Jw_tmp;
                  delta_Jw = 100;
              else
                  delta_Jw = abs(Jw-Jw_tmp);
                  Jw = Jw_tmp;
              end
    end
    Jw_vector =[Jw_vector Jw]; %存储每次训练次数的Jw
    delta_Jw_vector = [delta_Jw_vector delta_Jw]; %存储每次训练次数的Jw前后差值
   
    %反向的第0步，做一些备份和预计算
    Out_layer_compare = -Out_layer_sti+Target_add(randomindex,:);%1个样本，每一行三个元素代表和001/010/100的差距
    w_hj_backoff = w_hj;
    w_ih_backoff = w_ih;
    conditional_back1 = 0;
    conditional_back2 = 0;
    ...
    for Oi = 1:1:Output_J
        ...
        for Hi = 1:1:H
            ...
            for Alli = 1   %这里根据All代码直接修改，所以未对Alli结构修改
    ...
    for Hi = 1:1:H
        ...
        for Ii = 1:1:Input_I
            ...
            for Alli = 1
                ...
                delta_wih = delta_wih + delta_wih_Jpart*eta*(1-Hidden_layer_sti(Alli,Hi)*Hidden_layer_sti(Alli,Hi))*All_add(randomindex,Ii);
...
%%  用于检测所有样本是否归类正确
    THidden_layer_add = zeros(9,H);  %30个样本，存储每个隐含层神经元的bias+加权值
    THidden_layer_sti = zeros(9,H);  %30个样本，存储每个隐含层神经元的激励函数值
    TOut_layer_add = zeros(9,Output_J);%30个样本，输出层神经元的加权值
    TOut_layer_sti = zeros(9,Output_J);%30个样本，输出层神经元的激励值
    for TAlli = 1:1:9
         %前向的第一步，计算隐含层得到的net_h和激励值，隐含层用tanh函数
              for THi = 1:1:H
...
correct_rate = right/9;%------------------------------------------------------------测试集的测试精度
false_rate = 1 - correct_rate;%-----------------------------------------------------测试集的错误分类率
explode = [1,0];
name = {'true','false'};
    
figure(1)
subplot(2,2,1);
plot(Jw_vector([1:length(Jw_vector)]));title('随机单样本更新的损失函数随训练次数变化');ylabel('损失函数');xlabel('训练次数');
subplot(2,2,2);
plot(conditional_back_vector([2:length(conditional_back_vector)]));title('随机单样本更新的循环条件变化随训练次数变化');ylabel('循环条件变化');xlabel('训练次数');
subplot(2,2,[3,4]);
